...
## 3.2 加入集群
```
[root@node1 ~]# kubeadm join 192.168.10.130:6443 --token zac70e.6aqzqc3g4zytxzmf --discovery-token-ca-cert-hash sha256:78b86fcae0bb7101365495507c38f260b2729f012b17e6f577272164bc3781bf --ignore-preflight-errors=Swap
```
# 四、node2节点
配置和master几点相差无几,拉取镜像不同,kubeadm操作命令不同
## 4.1 拉取镜像
...
NAME         STATUS   ROLES    AGE     VERSION
k8s-master   Ready    master   18m     v1.14.2
k8s-node     Ready    <none>   9m24s   v1.14.2
[root@k8s-master ~]# kubectl label node k8s-node node-role.kubernetes.io/worker=worker
node/k8s-node labeled
[root@k8s-master ~]# kubectl get nodes
NAME         STATUS   ROLES    AGE   VERSION
k8s-master   Ready    master   20m   v1.14.2
k8s-node     Ready    worker   10m   v1.14.2
```